...
export class WebTracer {
  ...
  static init(config?: TraceConfig, spanProcessor?: SpanProcessor) {
    ...
    WebTracer.processor = spanProcessor || new BatchSpanProcessor(WebTracer.getExporter())
    WebTracer.provider = new WebTracerProvider({
      spanProcessors: [WebTracer.processor]
    })
    WebTracer.provider.register({
