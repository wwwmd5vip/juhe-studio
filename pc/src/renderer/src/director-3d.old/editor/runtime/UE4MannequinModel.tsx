...
export function UE4MannequinModel({
  ...
  // Track the current scene for cleanup on unmount.
  // The scene is a clone of the cached GLTF (via cloneSkeleton),
  // and isolateAndTintUE4MannequinMaterials clones materials before tinting.
  // R3F does not auto-dispose <primitive> objects, so we must dispose manually.
  const sceneRef = useRef(scene);
  sceneRef.current = scene;

  useEffect(() => {
    return () => {
      sceneRef.current?.traverse((child: Object3D) => {
