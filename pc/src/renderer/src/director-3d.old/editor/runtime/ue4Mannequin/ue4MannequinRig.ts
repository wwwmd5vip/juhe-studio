...
/**
 * UE4 Mannequin bone rotation functions.
 *
 * The GLB uses 3ds Max Biped bone naming. The root joint and pelvis both
 * carry a [-0.5,-0.5,-0.5,0.5] quaternion (a 90° compound rotation), so
 * bone local axes do NOT align with world axes:
 *
 *   local X → world Y (up)
 *   local Y → world Z (forward)
 *   local Z → world -X (sideways, left)
 *
 * We map control values (pitch/yaw/roll in world-friendly degrees) to
 * the correct local-axis Euler components.
 */
...
function ue4SpineRotation(
  ...
  // pitch (forward/back) → local Z (negated for correct forward direction)
  // yaw   (turn left/right) → local X
  // roll  (side lean) → local Y
  return [
...
function ue4ShoulderRotation(
  ...
  // twist (arm rotation along bone axis) → local X
  // spread (arm abduction, away from body) → local Y
  // pitch (arm forward/back) → local Z (negated for correct forward direction)
...
function ue4HipRotation(
  ...
  // twist (leg rotation along bone axis) → local X
  // spread (leg abduction) → local Y (NOT negated — both hips use same sign convention)
  // pitch (leg forward/back) → local Z
  const spread = controls[`${prefix}.spread`] ?? 0;
...
function ue4LimbBendRotation(
  ...
  // Elbow/knee flexion → local Z (negated for natural bending direction)
