export type ModelLibraryCategoryId = "primitives" | "convenience" | "home" | "outdoor" | "tools" | "my-models";
...
export const MODEL_LIBRARY_CATEGORIES: ModelLibraryCategory[] = [
  { id: "primitives", label: "基础几何", directoryName: "" },
...
/**
 * 内置程序化模型（无需外部文件）
 * 点击时动态生成 GLB 数据并添加到场景
 */
export const BUILT_IN_MODELS: Array<{
  id: string
  name: string
  type: 'box' | 'sphere' | 'cylinder' | 'cone' | 'plane' | 'torus'
}> = [
  { id: 'primitive-box', name: '立方体', type: 'box' },
  { id: 'primitive-sphere', name: '球体', type: 'sphere' },
  { id: 'primitive-cylinder', name: '圆柱', type: 'cylinder' },
  { id: 'primitive-cone', name: '圆锥', type: 'cone' },
  { id: 'primitive-plane', name: '平面', type: 'plane' },
  { id: 'primitive-torus', name: '圆环', type: 'torus' },
];
...
function createPrimitiveMesh(type: string): THREE.Mesh {
  ...
  switch (type) {
    ...
    case 'torus': geometry = new THREE.TorusGeometry(0.4, 0.15, 16, 32); break
    case 'sphere':
    default: geometry = new THREE.SphereGeometry(0.5, 32, 32); break
  }
  const material = new THREE.MeshStandardMaterial({ color: '#888888', metalness: 0.1, roughness: 0.7 })
  return new THREE.Mesh(geometry, material)
}

const builtInUrlCache = new Map<string, string>()

export async function getBuiltInModelUrl(id: string): Promise<string> {
  if (builtInUrlCache.has(id)) return builtInUrlCache.get(id)!
  const model = BUILT_IN_MODELS.find((m) => m.id === id)
  if (!model) throw new Error(`Unknown built-in model: ${id}`)

  const mesh = createPrimitiveMesh(model.type)
