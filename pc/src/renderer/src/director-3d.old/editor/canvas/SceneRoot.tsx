...
import { Box3, Matrix4, Mesh, Quaternion, Vector3, type Group, type Object3D } from "three";
...
function disposeThreeResources(obj: Object3D): void {
  obj.traverse((child) => {
...
function NormalizedImportedObject({ object }: { object: Object3D }) {
  ...
  const { clone, normalization } = useMemo(() => {
    // Dispose previous clone when the source object changes
    if (cloneRef.current) {
      disposeThreeResources(cloneRef.current)
    }
    const clonedObject = object.clone(true);
    clonedObject.updateMatrixWorld(true);
    cloneRef.current = clonedObject
  ...
  // Dispose cloned resources on unmount to prevent GPU memory leaks
  useEffect(() => {
    return () => {
      if (cloneRef.current) {
        disposeThreeResources(cloneRef.current)
