...
export function ViewportToolbar({
  ...
  // 处理内置模型点击（异步生成 GLB URL）
  async function handleBuiltInModelClick(modelId: string, modelName: string) {
    try {
      const url = await getBuiltInModelUrl(modelId)
      addImportedAsset({
        kind: "prop",
        assetSource: "library",
      ...
      console.error('[Director3D] Failed to create built-in model:', err)
    }
  }
  ...
  const actions: ToolbarAction[] = [
    ...
    { label: "删除选中", icon: Trash2, onClick: () => deleteSelectedObject(), disabled: !selectedObjectId && selectedObjectIds.length === 0 },
  ...
  const activeModelLibraryItems =
    activeModelLibraryCategoryId === "my-models"
      ? myModelLibraryItems
      : activeModelLibraryCategoryId === "primitives"
        ? BUILT_IN_MODELS.map((m) => ({
            categoryId: "primitives" as const,
            fileName: `${m.id}.glb`,
            id: m.id,
            name: m.name,
            url: "",
      ...
      {modelLibraryOpen ? (
        ...
        >
          ...
          {activeModelLibraryItems.length === 0 ? (
            <div className="model-library-empty-state object-search-empty-state" role="status" aria-label="模型库为空">
              <span className="object-search-empty-icon" data-testid="my-models-empty-icon">
                <Boxes aria-hidden="true" size={16} strokeWidth={1.8} />
              </span>
              <span>{activeModelLibraryCategoryId === "my-models" ? "暂无任何模型" : "预置模型库需要下载模型包"}</span>
              ...
              {activeModelLibraryItems.map((item) => (
                activeModelLibraryCategoryId === "my-models" ? (
                  ...
                  <button
                    ...
                    onClick={() => {
                      // 内置模型需要异步生成 URL
                      if (!item.url && item.id.startsWith('primitive-')) {
                        handleBuiltInModelClick(item.id, item.name)
                      } else {
