...
import type { CanvasNode, ViewportTransform } from './types'
...
export function MiniMap({ nodes, viewport, viewportSize, onViewportChange }: MiniMapProps) {
  ...
  const updateViewportFromEvent = useCallback(
    ...
    [toWorld, onViewportChange, viewportSize, viewport.k]
  )

  return (
    <div
      className='absolute bottom-20 left-6 z-50 overflow-hidden rounded-lg border shadow-2xl backdrop-blur-sm'
      style={{
        width,
        height,
    ...
    >
      ...
      >
        {nodes.map((node) => {
          ...
          const color = NODE_COLORS[node.type] || theme.node.muted
          return (
            <div
              key={node.id}
